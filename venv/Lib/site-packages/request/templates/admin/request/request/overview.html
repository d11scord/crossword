...
{% block breadcrumbs %}
    {% if request and request.user %}
        <div class="breadcrumbs">
            <a href="{% url "admin:index" %}">{% trans "Начало" %}</a> &rsaquo;
            <a href="{% url "admin:app_list" "request" %}">{% trans "Запрос" %}</a> &rsaquo;
            <a href="{% url "admin:request_request_changelist" %}">{% trans "Запросы" %}</a> &rsaquo;
            {% trans "Статистика" %}
        </div>
    {% endif %}
{% endblock %}
...
<div style="width:920px;">
    <div class="module" style="clear: both;">
        <table style="width: 100%;">
            <caption>{% trans "Статистика посещений" %} ( <a href="javascript:loadTrafficGraph(7);">{% trans "Неделя" %}</a> | <a href="javascript:loadTrafficGraph(30);">{% trans "Месяц" %}</a> | <a href="javascript:loadTrafficGraph(365);">{% trans "Год" %}</a> )</caption>
        </table>
